...
const N=await p.evaluate(()=>document.querySelectorAll('[data-testid^="dev-"]:not([data-testid="dev-toggle"]):not([data-testid="dev-menu"])').length);
async function preset(i){await p.goto(U);await p.waitForTimeout(280);await p.click(toggleSel);await p.click('[data-testid^="dev-"]:not([data-testid="dev-toggle"]):not([data-testid="dev-menu"]) >> nth='+i);await p.waitForTimeout(320);}
const res={};
