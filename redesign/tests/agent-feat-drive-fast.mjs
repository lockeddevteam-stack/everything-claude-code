...
for(const s of screens){
  ...
  await page.goto(base); await page.waitForTimeout(170);
  ...
  for(const url of entries){
    ...
    for(const it of top){
      await page.goto(url); await page.waitForTimeout(140);
      ...
      if(ch){
        ...
        for(const jt of fresh){
          await page.goto(url); await page.waitForTimeout(140);
          await page.evaluate(clickSel,it); await page.waitForTimeout(170);
...
fs.writeFileSync('/tmp/drivefast-'+screens.join('_').replace(/[^a-z]/g,'')+'.json',JSON.stringify(out,null,1));
await browser.close();
