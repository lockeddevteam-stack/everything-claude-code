...
```sh
sh redesign/tests/gate.sh          # 23 suites, ~4.5 min
sh redesign/tests/run-parallel.sh  # everything, 57 suites, ~25 min
```

These are standalone `.mjs` files, not the Playwright-config harness above:
each one stands up its own server on port 0 and drives the assembled build
at `10-final/locked-app.html`. `gate.sh` rebuilds first, so a build that
does not assemble fails before anything else runs. Set `JOBS` to control
parallelism — the default is `nproc`, and four Chromiums at once will get
OOM-killed on a small box, so `JOBS=2` for the full run.

### What each guard protects

Every suite here exists because something real broke. The class matters
more than the case: a guard that only caught its own bug would have been
a changelog entry instead.

| Suite | The class it holds |
|---|---|
| `day-to-day` | A day's actual use: log a workout, log food, come back |
| `first-run` | A brand new account, from sign-up to a saved first workout |
| `setup-recovery` | Setup interrupted, gone back to, and corrected |
| `session-persist` | A workout survives a reload mid-set |
| `past-session` | An old session opens and reads right |
| `empty-state` | A screen with nothing on it is still a screen |
| `no-dead-ends` | Every screen can be left by a control that is on it |
| `crossings` | Every declared navigation actually goes somewhere |
| `bad-data` | A wrong-shaped key, AND one unreadable row in a good list |
| `one-of-everything` | An account where every count is 1 — every new account |
| `singulars` | Ids, edit dates and plurals at their boundaries |
| `pounds` | The reader who works in pounds, in both directions |
| `late-night` | Dates west of Greenwich, where local and UTC disagree |
| `rest-clock` | Clocks derived from deadlines, correct after a shut app |
| `gestures` | Swipe-to-delete and drag-to-reorder, at v6's thresholds |
| `destructive` | What a delete takes WITH it, and whether Undo means it |
| `hostile-text` | A typed name is text, not markup, on every screen |
| `escaping` | The same, read statically, where a seed cannot reach |
| `hostile-server` | A pull that answers badly damages nothing |
| `coach-apply` | A model's write lands once, beside the reader's work |
| `offline` | A whole session logged with every request failing |
| `cloud-contract` | The seam against the backend that actually exists |
| `product-build` | The shipped build, with no fixtures in it |

### What is NOT covered

Written down because an untested area that nobody has named reads exactly
like a tested one.

- **The live backend.** supabase.co and workers.dev are unreachable from
  this container, so every backend suite runs against a local mirror built
  from the real schema and the deployed Worker. A mirror cannot prove the
  live server behaves the way it models.
- **Real iOS and Android.** Everything runs in Chromium at 393×852 with
  touch emulated. Safari's own quirks — bfcache, momentum scrolling, the
  keyboard's effect on viewport units — are not exercised.
- **Push notifications actually arriving.** The rest timer hands its
  deadline to the service worker, and the worker is asked to post a
  notification. Whether a browser chose to keep that worker alive is not
  something a harness can assert, and asserting it would prove nothing.
- **Two devices at once.** The conflict rule is tested one row at a time
  against a mirror; two phones writing the same key in the same second is
  not simulated.
- **Long-run storage pressure.** Nothing fills localStorage to its quota
  and checks what gives way first.

A green gate means the classes above hold. It does not mean the app has no
bugs, and no suite here should ever be described as proving that.
