...
/* THE DEV STATE SWITCHER IS NOT A PRODUCT CONTROL. Every screen carries
   ...
   IT ADOPTED SEVENTEEN OF THEM. Onboarding's switcher is the odd one out:
   it is a panel with an open button rather than a toggle with a menu, and
   it is named for what it is -- dev-open, dev-panel -- while the other
   seventeen are named dev__toggle and dev__menu. This rule listed the
   seventeen names and nothing else, so the one screen it missed was the
   first screen a new install ever sees: a 44px inspector button, live, in
   the corner of the welcome screen, opening a list of twenty-eight
   internal states. Hiding by name is what let one screen out; the last
   two selectors close it by shape, so a switcher named something new
   still cannot ship. */
.dev, .dev__toggle, .dev__menu, .dev-open, .dev-panel,
[data-testid="dev-toggle"], [data-testid="dev-menu"],
[data-testid="dev-open"], [data-testid="dev-panel"],
[data-testid^="dev-"][aria-controls],
[data-testid^="dev-"][aria-label="Screen and state inspector"] { display: none !important; }

/* AND NEITHER IS THE DEMO'S OWN CHROME. The Screens pill and the Back
   button are the harness, not the app. On a phone-width window they are
   parked off the canvas at translateX(-100% + 8px) -- deliberate, for the
   demo, where 8px of reachable scaffolding beats a pill sitting on the
   design it exists to show. The product build inherited the parking and
   not the reason for it: eight pixels of a debug control down the left
   edge of every screen, at z-index 40, and elementFromPoint(4, y) returned
   demo-back rather than the app. It was not decorative. Tapping that
   sliver navigated -- it took a session from #/profile/settings back to
   #/profile/onboarding, which to anyone holding the phone is the app
   throwing away where they were because their thumb brushed the edge.

   display:none rather than a transform, so it is out of hit-testing too. */
.demo-chrome, .demo-index { display: none !important; }
` : ''}
</style>
