...
Polyline, 6 points, accent stroke. No axes, gridlines or y labels: `y = 110 − (e1−mn)/rg*100` rescales to the visible min/max, pinning the lowest point to the floor and the highest to the ceiling whatever the spread. X is `i*(vw−20)/(n−1)`, **index-based, not time-based**, while the corner labels are dates, so a 3-day and a 10-day gap draw identically. A reader gets two facts: `6 sessions`, and `91.75 kg` printed in the header, not on the plot.
...
Contrast, list view, per distinct style (74 nodes):

Threshold 4.5; `PR VAULT` large, 3.
...
Motion: real `mouse.down` read at 100ms — 5/5 change (accent button, card, Back, input, range chip; `scale(.97)` except the input's `rgba(24,24,27,.8)→.89`). Two transition rows (`background-color, box-shadow .24s ease` ×51; a 6-prop `.3s` ×1), zero `animation-name`, nothing decorative. No `:disabled`/`aria-disabled` rule in L29165–30580. Dark: L* 0 → 10.3 → 18.1, strictly increasing; one accent glow.

## 5. Rubric — Design

| criterion | score | evidence |
|---|---|---|
| Hierarchy | 3 | squint of `-populated.png`: one dominant element, but it is the tertiary action; 9 identical 353×119.5/r16 cards |
...
| Component consistency | 1 | 8 radii (7–16); 3 chip variants; card r16 list vs r14 detail |
...
| HIG fit | 1 | Buttons (42px primary, 19px chips, glow), Typography (9 sizes), Layout (62% dead chart width): 3 sections, `-detail.png` |
...
| # | verdict | measurement |
|---|---|---|
| 1 | pass | 5-s test names it; one accent-filled control. Shortfall: the wrong action |
...
| 6 | fail | empty = `No PRs yet`: 3 words, no instruction, no action control (`-empty.png`). Error N/A per index.md |
