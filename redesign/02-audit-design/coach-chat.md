...
## 2. Callouts
Nine numbered markers with a printed legend on `coach-chat-annotated.png`.
...
- **Accent**: four accent surfaces at once (logo, active tab, bubble fill, FAB); none is the primary action.

## 7. Keep, fix, cut

- **Keep**: composer — Enter sends, textarea grows 48→120px, thread pinned to bottom on entry, Send `disabled` on empty.
...
- **Fix**: give replies a real surface (`--color-card`, 16px) so both speakers are one component, two states.
