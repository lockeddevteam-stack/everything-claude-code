...
Surface: SplitBuilder, AISplitBuilder, ConvertToSplitModal. Captures: `split-builder-populated(-full/-ai).png`, `-empty.png`, `-error.png`; convert sheet re-captured live.
...
**Convert sheet.** Two 353x71 options as equal peers in two accents (orange, info blue), each with an ➕; Cancel 353x41; no grabber; 20px radius against 16px page cards; no entrance transition.
...
Targets: 42 interactive, **11 under 44x44 (73.8% pass)** — Back 22x27; rename `p[role=button]` 175x22 x3; "+ Exercise" 157x33 x3; "📝 Block" 155x33 x3; Cancel 353x42; sheet Cancel 353x41. No horizontal overflow (`scrollWidth` 393 = `clientWidth`).
...
Type: **7 sizes** — 11, 12, 13, 14, 15, 18, 32. Weights 400/600/700/800; 800 on the 14px index and 18px SAVE SPLIT; 700 on the 15px day name. Line-height explicit on 6 styles, `normal` on 8. Tabular figures 0 of 3.
...
Pressed (real `mouse.down` at 100ms): SAVE SPLIT, Add, + Exercise, Remove-exercise, Cancel all scale to .97 — **5/5**. `button:disabled{opacity:.4}`, measured 0.4 on SAVE SPLIT in `-empty.png`.

## 4. Rubric scores

| criterion | score | evidence |
|---|---|---|
| Hierarchy | 2 | squint winner is the global FAB, then the 32/800 title; zero primary-styled page controls above the fold; SAVE SPLIT 1,123px down, flanked by a full-width Cancel (`-populated-full.png`) |
...
| Motion and feedback | 4 | 5/5 pressed at 100ms; zero page animations, zero decorative rows; shortfall — the sheet has no entrance transition and adding a day does not animate (measured) |
...
| 6 | Empty/error copy | Fail | `-empty.png`: no instruction sentence, three action controls; `-error.png` (worker aborted) renders the canned question, no error text |
...
- Cards: 16px day card, dashed add-day card, 20px sheet — three treatments where the system defines one.
- Accent: the sheet promotes `--color-info` to a co-equal second accent, against the one-accent rule.
...
- **Fix** Save: pin it as a persistent primary; make Back a dismiss with an unsaved-changes confirmation (today it discards silently at 22x27px).
