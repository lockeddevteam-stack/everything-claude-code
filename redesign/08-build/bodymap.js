  ...
  function mount(host, opts) {
    ...
    api.showParts = function (gid, view) {
      ...
      if (spec.kind === 'muscles') {
        ...
        function cutClip(uid, cut, keepAbove, flip, lo, hi) {
        ...
        spec.names.forEach(function (nm, i) {
          ...
          sides.forEach(function (S2) {
            ...
            var node = paint;
            if (bot) {
              var wrapB = api.el('g', { 'clip-path': 'url(#' + cutClip(uid + '-pb', bot, true, S2.flip, S2.lo, S2.hi) + ')' });
              wrapB.appendChild(node); node = wrapB;
            }
            if (top) {
              var wrapT = api.el('g', { 'clip-path': 'url(#' + cutClip(uid + '-pt', top, false, S2.flip, S2.lo, S2.hi) + ')' });
            ...
            [[bot, true], [top, false]].forEach(function (pair, pi) {
              ...
              var bclip = api.el('g', { 'clip-path': 'url(#' +
                cutClip(sid, cut, !isBot, S2.flip, S2.lo, S2.hi) + ')' });
            ...
            var hnode = hit;
            if (bot) {
              var hb = api.el('g', { 'clip-path': 'url(#' + cutClip(uid + '-b', bot, true, S2.flip, S2.lo, S2.hi) + ')' });
              hb.appendChild(hnode); hnode = hb;
            }
            if (top) {
              var ht = api.el('g', { 'clip-path': 'url(#' + cutClip(uid + '-t', top, false, S2.flip, S2.lo, S2.hi) + ')' });
          ...
          /* WHERE THE CAMERA LOOKS. zoomTo frames one side only when a
             group is wider than half the figure -- two arms, two delts.
             A chest is one region in the middle and stays whole, so its
             labels belong on the midline where they have always sat.
             Keying this off MIRROR instead pushed them onto one
             pectoral of a muscle the camera shows both of. */
          var camPairs = box[2] > FIGURE[v][2] * 0.5;
          var labX = camPairs ? (b.x + mxv) / 2 : cx;
      ...
      parts.forEach(function (P) {
        ...
        if (P.at) {
          labels.push({ x: Math.max(VBOX.x + 42, Math.min(P.at.x, VBOX.x + VBOX.w - 42)),
                        y: P.at.y, name: P.name, i: P.i, gid: P.gid });
          return;
        }
