...
(function (global) {
  ...
  var MIRROR = { shoulders: true, biceps: true };
  var FIBRE = {
  ...
  function mount(host, opts) {
    ...
    api.showParts = function (gid, view) {
      ...
      if (spec.kind === 'muscles') {
        ...
        var rad = (BAND_AXIS[gid] || 0) * Math.PI / 180;
        var cx = b.x + b.w / 2, cy = b.y + b.h / 2;
        /* The axis the bands are measured along: the muscle's height
           turned by the cut angle. Extended by the width times the sine,
           or a steep cut runs out of muscle before it runs out of band. */
        /* Long enough that a turned cut still crosses the whole belly,
           short enough that the end bands are not mostly empty space:
           the corners the extension buys are outside the muscle anyway. */
        var reach = 0.88 * (Math.abs(b.h * Math.cos(rad)) + Math.abs(b.w * Math.sin(rad))) / 2;
        var ax = { x1: cx - Math.sin(rad) * reach, y1: cy - Math.cos(rad) * reach,
                   x2: cx + Math.sin(rad) * reach, y2: cy + Math.cos(rad) * reach };

        spec.names.forEach(function (nm, i) {
          var uid = 'prt-' + api.uid + '-' + v + '-' + gid + '-' + i;
          var t0 = i / bands, t1 = (i + 1) / bands;
          /* How far the blend reaches. A third of a band each side turned
             out to BE the wash the comment warned about: with three
             heads the feathers met in the middle and the tones cancelled.
             A fifth keeps a soft boundary and leaves each head its own
             value. */
          var f = 0.16 / bands;
          var stops = [];
          var at = function (o, a) { stops.push([Math.max(0, Math.min(1, o)), a]); };
          if (i === 0) at(0, 1); else { at(t0 - f, 0); at(t0 + f, 1); }
          if (i === bands - 1) at(1, 1); else { at(t1 - f, 1); at(t1 + f, 0); }

          var mask = api.el('mask', { id: uid + '-m', maskUnits: 'userSpaceOnUse' });
          var addRamp = function (suffix, x1, y1, x2, y2, rect) {
            var mg = api.el('linearGradient', { id: uid + suffix, gradientUnits: 'userSpaceOnUse',
                                                x1: n2(x1), y1: n2(y1), x2: n2(x2), y2: n2(y2) });
            stops.forEach(function (st) {
              mg.appendChild(api.el('stop', { offset: String(Math.round(st[0] * 1000) / 1000),
                                              'stop-color': '#fff', 'stop-opacity': String(st[1]) }));
            });
            api.defs.appendChild(mg);
            mask.appendChild(api.el('rect', {
              x: n2(rect.x), y: n2(rect.y), width: n2(rect.w), height: n2(rect.h),
              fill: 'url(#' + uid + suffix + ')'
            }));
          };
          if (mirrored) {
            /* Each side gets its own ramp, running from that side's outer
               edge towards the body's middle, so t=0 is lateral on both
               limbs and a name means the same thing on each. */
            var mx = midX(v);
            addRamp('-mgR', mx + (b.x + b.w - mx), cy, mx, cy,
                    { x: mx, y: b.y - 16, w: (b.x + b.w) - mx + 16, h: b.h + 32 });
            addRamp('-mgL', b.x - 16, cy, mx, cy,
                    { x: b.x - 16, y: b.y - 16, w: mx - (b.x - 16), h: b.h + 32 });
          } else {
            addRamp('-mg', ax.x1, ax.y1, ax.x2, ax.y2,
                    { x: b.x - 16, y: b.y - 16, w: b.w + 32, h: b.h + 32 });
          }
          api.defs.appendChild(mask);
          ...
          g.appendChild(inner);
          /* The striation, over the same shape in the same place. Its own
             copy of the paths rather than a filter, because a filter
             would soften the muscle's outline along with it. */
          var deg = fibreAngle(gid, v, nm, i, bands);
          var fibR = fibrePattern(api, uid + 'r', deg);
          var fibL = fibrePattern(api, uid + 'l', -deg);
          var over = api.el('g', { class: 'part__fib', transform: fitAttr(v) });
          (ART[v].groups[gid] || []).forEach(function (d) {
            over.appendChild(api.el('path', {
              d: d, stroke: 'none',
              fill: 'url(#' + (sideOf(d, v) < 0 ? fibL : fibR) + ')'
            }));
          });
          g.appendChild(over);
          /* PAINT SOFTLY, HIT SHARPLY. A mask feathers the boundary and
             does nothing to pointer events, so all three bands covered
             the whole pectoral and the last one drawn took every tap:
             Upper was unreachable at any point on the muscle. The paint
             keeps the gradient; the target is a transparent copy clipped
             to the band's own stripe, which is the one thing that has to
             have an edge. */
          var cid = uid + '-c';
          var cp = api.el('clipPath', { id: cid, clipPathUnits: 'userSpaceOnUse' });
          /* THE TRANSFORM GOES ON THE SHAPE. A clipPath may hold shapes,
             text and use; a <g> inside one is ignored, which clips
             everything away rather than nothing -- the whole hit layer
             disappeared and every band became untappable. */
          var span = (b.h + 32) / bands;
          cp.appendChild(api.el('rect', {
            x: n2(b.x - 16 - b.w), y: n2(b.y - 16 + span * i),
            width: n2(b.w * 3 + 32), height: n2(span + 0.4),
            transform: 'rotate(' + n2(BAND_AXIS[gid] || 0) + ' ' + n2(cx) + ' ' + n2(cy) + ')'
          }));
          api.defs.appendChild(cp);
          var hit = api.el('g', { class: 'part__hit', 'clip-path': 'url(#' + cid + ')' });
      ...
      parts.forEach(function (P) {
        ...
        Array.prototype.forEach.call(shapes, function (sh) {
          ...
          if (paired && (b.x + b.width / 2) > artMid) return;
          take(b); any = true;
        });
        if (!any) {
        ...
        if (paired && parts.length > 1) {
          ly = fr.y + fr.h * (0.30 + (0.40 * P.i) / (parts.length - 1));
        }
        ...
        labels.push({ x: lx, y: ly, name: P.name, i: P.i, gid: P.gid });
      });
