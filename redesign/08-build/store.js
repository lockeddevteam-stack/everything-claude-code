...
(function (g) {
  ...
  var SCALAR_KEYS = asSet([
    ...
    'lk_foodUnit',
    /* What unit the shipped app's stored weights are in, written down once
       and never re-derived. See v6WeightUnit. */
    'lk_v6WeightUnit'
  ]);
  ...
  var SYNC_KEYS = [
    ...
    /* Both devices on one account must agree what a number written in
       June meant, so this travels with the history it describes. */
    'lk_v6WeightUnit',
  ...
  /* ---- WHAT UNIT THE OLD DATA IS ACTUALLY IN ---------------------------
     The shipped app did NOT store lifting weights in kilograms. It stored
     them in whatever unit the account logged in, and recorded which that
     was in lk_weightStorageUnit -- its own comment says so: "for an
     account that logged in lb with conversion off it is lb".

     Migration 3 below read those numbers as kilograms. For an account
     that lifts in pounds that relabels every set: a 135 lb bench becomes
     135 kg, and the screen, converting kilograms to pounds to show it,
     renders 298. Every weight in the account reads 2.2 times what was
     lifted. It is not a display fault -- the stored number is wrong -- and
     it is invisible to anyone testing in kilograms, which is everyone
     here.

     The answer is written down once, under this build's own key, and
     never re-derived. The old keys can be changed by the old app on
     another phone; what a number written in June meant does not change
     because a switch moved in September. */
  var LB_PER_KG = 2.20462;

  function v6WeightUnit(ST) {
    var known = ST.get('lk_v6WeightUnit', null);
    if (known === 'kg' || known === 'lb') return known;

    /* The shipped app's own record of it, in either form it is stored. */
    var saved = ST.get('lk_weightStorageUnit', null);
    if (typeof saved === 'string') saved = saved.replace(/^"|"$/g, '');
    if (saved !== 'kg' && saved !== 'lb') {
      try {
        var raw = window.localStorage.getItem('lk_weightStorageUnit');
        if (raw) {
          raw = String(raw).replace(/^"|"$/g, '');
          if (raw === 'kg' || raw === 'lb') saved = raw;
        }
      } catch (e) {}
    }
    ...
    if (saved === 'kg' || saved === 'lb') unit = saved;
      ...
      /* The same derivation the old app used when it had no record either,
         so two devices on one account reach the same answer. */
      var conv = ST.get('lk_unitConversion', null);
      var prof = ST.get('lk_profile', null) || {};
      unit = conv ? 'kg' : (prof.useKg === false ? 'lb' : 'kg');
    }
    ST.set('lk_v6WeightUnit', unit);
    return unit;
  }

  function toKg(n, unit) {
    if (typeof n !== 'number' || !isFinite(n)) return n;
    return unit === 'lb' ? Math.round((n / LB_PER_KG) * 1000) / 1000 : n;
  }

  /* A session total carries its own unit in the string: "14705 lb",
     "3608 kg". That is better evidence than any setting, because it
     records what was on screen the day it was saved, so it is read from
     the row itself rather than from the account. */
  function volKg(v, fallbackUnit) {
    if (typeof v === 'number') return toKg(v, fallbackUnit);
    if (v == null) return null;
    var text = String(v);
    var x = parseFloat(text.replace(/[^0-9.\-]/g, ''));
    if (!isFinite(x)) return null;
    var unit = /lbs?\b/i.test(text) ? 'lb' : (/kgs?\b/i.test(text) ? 'kg' : fallbackUnit);
    return toKg(x, unit);
  }
  ...
  var MIGRATIONS = [
    ...
    function (ST) {
      ...
      /* THE UNIT THOSE NUMBERS WERE WRITTEN IN. Read before anything is
         converted; see v6WeightUnit above for why this was the whole
         defect. */
      var unit = v6WeightUnit(ST);
      var n2 = function (v) {
      ...
      var out = rows.map(function (w) {
        ...
        r.kind = w.kind || 'lift';
        r.sets = n2(w.sets);
        r.kg = w.kg !== undefined ? toKg(n2(w.kg), unit) : volKg(w.vol, unit);
        r.exercises = (w.exercises || []).map(function (ex) {
          return {
            ...
            sets: (ex.sets || []).map(function (st) {
              ...
              return {
                kg: toKg(n2(st.w), unit), reps: n2(st.r), rir: n2(st.rir),
                w: st.w, r: st.r,
    ...
    /* 11 — PUT THE POUNDS BACK.

       Migration 3 shipped reading every stored set weight as kilograms.
       The shipped app stored them in whatever unit the account lifted in,
       so for anybody logging in pounds every set in their history was
       relabelled: 135 lb became 135 kg, and the screen, converting to
       pounds to show it, rendered 298. Their entire training history read
       2.2 times heavier than anything they had ever lifted. A rewritten
       migration cannot reach them -- theirs already ran.

       It is repairable, and only because migration 3 kept the original
       `w` beside the `kg` it derived. Every damaged figure still has its
       source string sitting next to it, so this does not try to guess
       which numbers were doubled and undo them; it recomputes each one
       from the original, in the unit that original was written in. Run it
       twice and you get the same answer, because the input never changes.

       A set logged in THIS build has a kg and no w, and is never touched.
       A session total is read from its own `vol` string, which carries
       its unit with it. */
    function (ST) {
      if (!ST.touched('lk_history')) return false;
      var rows = ST.get('lk_history', null);
      if (!Array.isArray(rows) || !rows.length) return false;
      var unit = v6WeightUnit(ST);
      /* Nothing to put right on an account that always lifted in kilos,
         and nothing to get wrong either -- but the walk is skipped rather
         than run as nineteen no-ops. */
      if (unit !== 'lb') return false;
      ...
      rows.forEach(function (w) {
        ...
        /* The session total, from the string that names its own unit. */
        if (w.vol !== undefined) {
          var v = volKg(w.vol, unit);
          if (v !== null && v !== w.kg) { w.kg = v; touched = true; }
        }
        (w.exercises || []).forEach(function (ex) {
          (ex && ex.sets || []).forEach(function (st) {
            if (!st || typeof st !== 'object') return;
            /* No original beside it means this set was logged in this
               build, in kilograms, and is already right. */
            if (st.w === undefined || st.w === null || st.w === '') return;
            var right = toKg(num(st.w), unit);
            if (right === null) return;
            if (typeof st.kg !== 'number' || Math.abs(st.kg - right) > 0.001) {
              st.kg = right;
              touched = true;
            }
          });
