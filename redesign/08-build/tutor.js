...
(function (g) {
  ...
  var CSS =
    /* EVERY CLASS HERE IS NAMESPACED. components.css already owns .tut__card
       and .tut__tag for the tutorial SCREEN, and that sheet is at document
       level in the assembled app where this layer also lives: .tut__card's
       width:100% beat this card's right offset and pushed it 16px off the
       screen, and .tut__tag's secondary grey beat the pointer tag's colour
       and failed contrast on the accent fill in the light theme. */
    ...
    '.lktut__spot{position:absolute;border-radius:14px;pointer-events:none;' +
    ...
    '.lktut__pulse{position:absolute;border-radius:14px;pointer-events:none;opacity:0;' +
    ...
    /* --text-on-accent, not a guess at one. The token is what the design
       system pairs with --accent in each theme, and the light theme's
       accent is a deep orange that near-black text fails against. */
    '.lktut__tag{position:absolute;transform:translateX(-50%);pointer-events:none;opacity:0;' +
      'background:var(--accent,#f97316);color:var(--text-on-accent,#0b0b0c);font-size:12px;' +
    ...
    '.lktut__ghost{position:absolute;width:31px;height:31px;margin:-15.5px 0 0 -15.5px;' +
    ...
    '.lktut__card{position:absolute;left:16px;right:16px;pointer-events:auto;' +
    ...
    '.lktut__card[data-at="bottom"]{bottom:16px}' +
    '.lktut__card[data-at="top"]{top:16px}' +
    '.lktut__where{display:flex;justify-content:space-between;font-size:11px;' +
      'letter-spacing:.08em;text-transform:uppercase;opacity:.6;margin-bottom:6px}' +
    '.lktut__say{margin:0;font-size:16px;line-height:1.35;font-weight:650}' +
    '.lktut__note{margin:6px 0 0;font-size:13.5px;line-height:1.4;opacity:.72}' +
    '.lktut__note[hidden]{display:none}' +
    '.lktut__row{display:flex;gap:8px;align-items:center;margin-top:12px}' +
    '.lktut__row button{min-height:var(--tap,44px);padding:0 14px;border-radius:12px;' +
      'font:inherit;font-weight:700;font-size:14px;cursor:pointer;border:0}' +
    '.lktut__row button[hidden]{display:none}' +
    ...
    '.lktut__skip{background:transparent;color:var(--text,#f2f2f4);' +
      'border:1px solid rgba(255,255,255,.22) !important}' +
    '.lktut__show,.lktut__go{margin-left:auto;background:var(--accent,#f97316);' +
      'color:var(--text-on-accent,#0b0b0c)}' +
    /* Reduced motion cuts rather than springs, and holds nothing back. */
    '#lk-tutor[data-reduced] .lktut__spot,#lk-tutor[data-reduced] .lktut__pulse,' +
      '#lk-tutor[data-reduced] .lktut__ghost{transition:none}' +
    '#lk-tutor[data-reduced] .lktut__pulse{animation:none}' +
  ...
  function build() {
    ...
    L.innerHTML =
      '<div class="lktut__spot" data-testid="tutor-spot"></div>' +
      '<div class="lktut__pulse" data-testid="tutor-pulse"></div>' +
      '<div class="lktut__tag" data-testid="tutor-tag"></div>' +
      '<div class="lktut__ghost" data-testid="tutor-ghost"></div>' +
      '<div class="lktut__card" data-testid="tutor-card" role="status" aria-live="polite">' +
        '<div class="lktut__where"><span class="lktut__mod" data-testid="tutor-module"></span>' +
          '<span class="lktut__count" data-testid="tutor-count"></span></div>' +
        '<p class="lktut__say" data-testid="tutor-say"></p>' +
        '<p class="lktut__note" data-testid="tutor-note"></p>' +
        '<div class="lktut__row">' +
          '<button type="button" class="lktut__skip" data-testid="tutor-skip">Skip</button>' +
          '<button type="button" class="lktut__show" data-testid="tutor-show" hidden>Show me</button>' +
          '<button type="button" class="lktut__go" data-testid="tutor-continue" hidden>Continue</button>' +
        '</div>' +
      '</div>';
    doc.body.appendChild(L);
    if (REDUCED) L.setAttribute('data-reduced', '1');

    q('.lktut__skip').addEventListener('click', function () { stop('skipped'); });
    q('.lktut__show').addEventListener('click', function () { perform(step()); });
    q('.lktut__go').addEventListener('click', function () { advance(); });
    return L;
  }
  ...
  function setSpot(el, pad) {
    var spot = q('.lktut__spot'), pulse = q('.lktut__pulse');
  ...
  function setPulse(on) { q('.lktut__pulse').style.opacity = on ? '1' : '0'; }

  function setTag(el, text) {
    var tag = q('.lktut__tag');
  ...
  function caption(st, mod, n, total) {
    q('.lktut__mod').textContent = mod || '';
    q('.lktut__count').textContent = total ? ('Step ' + n + ' of ' + total) : '';
    q('.lktut__say').textContent = st.say || '';
    var note = q('.lktut__note');
  ...
  function disarm() {
    ...
    q('.lktut__show').hidden = true;
    q('.lktut__go').hidden = true;
    q('.lktut__ghost').style.opacity = '0';
  }
  ...
  function armOn(st, el) {
    ...
    setTimeout(function () {
      ...
      /* NOBODY GETS STUCK. */
      S.idleT = setTimeout(function () { q('.lktut__show').hidden = false; }, IDLE_MS);
    }, REDUCED ? 0 : 240);
  }
  ...
  var API = {
    ...
    waiting: function () { return !!(L && !q('.lktut__go').hidden); },
