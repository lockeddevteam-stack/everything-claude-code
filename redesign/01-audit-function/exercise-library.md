...
| ADD TO WORKOUT | L7463-7476 | **dead from Library tab** | TrainHub passes no `onSelect` (L15056); handler L7861 guards `p.onSelect &&`, then closes. It is the sheet's primary in `-detail-modal.png` |
...
No per-step timestamp log exists (no `flow-<n>.json` in `tests/baseline/`); videos are at `baseline/artifacts/flows-01-*/video.webm`.
...
|---|---|---|---|
| Empty | `exercise-library-empty.png` | yes | GENERIC — "No results" alone, no next action |
| Loading | `exercise-library-loading.png` | **MISSING** | md5-identical (`a2c8a5f…`) to `-detail-modal.png`; test 2a fails and 2b finds no spinner, skeleton or busy control. The "Finding a form video…" branch (L7346) is below the fold |
| Error | `exercise-library-error.png` | yes (GIF box gone) | **PRESENT-BUT-WRONG** — a worker abort reads "No form video found for this exercise."; the GIF failure says nothing |
...
| Error recovery | 2 | `exercise-library-error.png`: cause unnamed, no retry, GIF failure silent (`onError` hides the img, L7318). Nothing lost — notes persist (L7091) |
