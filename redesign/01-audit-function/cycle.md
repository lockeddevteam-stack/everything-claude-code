...
| McNutritionCard | L24777 | working (phase-gated) | absent in follicular capture; menstrual/luteal/ovulatory only (L24783) |
...
| McSettings: lengths, irregular, birth control, discreet, cloud, export, delete-all | L24066 | working | `cycle-populated-settings-full.png` |
| McOnboarding, 5 steps | L22751 | working | `cycle-empty-onboarding.png`…`-step5.png` |
| McTrainBanner in Train Hub | L25064, render L15465 | working, narrow | fires only on today's `sym ≥2` or `energy ≤2`; text only, links back here |
...
## 3. Task walkthrough
No flow in `user-flows.md` touches this page and `flow-metrics.jsonl` has no cycle row, so no elapsed-ms or video evidence exists. Taps counted from the reach paths in `screenshots/index.md` plus the interactive crawl (`page-metrics.jsonl`, 10,735 ms, 20/35 controls).
...
| Log symptoms | card → "Symptoms" → chip → "Done" | 4 | four identical chips; two open the same sheet |
| See the month | card → "Show calendar" | 2 | header icon flips calendar→clock, no label change |
| Change settings | card → "Cycle settings" | 2 | crawl logged `overlays:[{after:"Cycle settings", closedBy:"unresolved"}]`; the sheet trapped it, 15/35 controls never exercised |
...
|---|---|---|---|
| Empty | `cycle-empty.png` | **MISSING** — test 2a: byte-identical to `cycle-populated.png` (md5 `a2ad5bb0…`), as are the `-full` and `-scrolled-2` pairs | Every base-view element derives from `lk_mcProfile`, and the patterns card already shows empty copy when populated, so clearing `lk_mcDays` changes nothing. The baseline empty assertion passes in both states — a false pass |
| Populated | `cycle-populated.png` | present, correct | ring, chips, training, science all render |

Band table: M=1, W=0 → **2**.

## 5. Baseline failures
Populated and empty passed; error N/A; 0 console/page errors. Non-failing defects:
- **D2** (blocks a task): Home cycle-card **LOG** intercepted by VoiceButton (L53718, fixed L54052; LOG L22735).
...
| Data correctness | 4 | Seed `lastStart` 2026-08-29, `cycleLen` 28 → day 11 ✓, "Sep 26 / in 18d" ✓, fertile "Sep 6 – Sep 12" ✓ (`cycle-populated-full.png`, `cycle-populated-calendar.png`). Shortfall: the calendar paints `flow≥1` as "Period" (L23877) while the engine counts one only at `flow≥2` (L21972), so seeded Sep 1–2 read as period days the predictions ignore |
...
**Fix**
- Empty state renders nothing distinguishable; build one (§4).
