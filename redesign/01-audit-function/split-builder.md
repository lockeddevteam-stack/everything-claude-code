...
Assemble or edit a named split — its days, and the exercises in each day — then save it to `lk_splits`.
...
`page-metrics.jsonl` L53: populated/empty/error **passed**; `nonResponders: []`, `blocked: []`, 0 console and page errors.

**The 21/40 gap is not dead UI.** All 19 unclicked elements sit in `skipped`, `reason: "destructive/allowlisted label"`: 3x `Remove this day`, 16x `Remove this exercise from the day`. Every non-destructive control responded. Cause: unconfirmed destructive actions L9390-9418, same class as D15.
...
|---|---|---|
| Purpose clarity | 3 | `split-builder-empty.png` states the job ("New Split", name, Add a day). `split-builder-populated.png` shows no title, name field or SAVE; four peer accents (+Exercise x3, Add) compete. Job inferable after a scan |
