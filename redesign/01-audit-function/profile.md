...
| Error recovery | N/A | No network call, no input (page-map 2.16); nothing can fail here |
...
- Badges: default-off, so they teach nothing; when on, six equal decorative tiles compete with the stats and justify no place (`profile-populated-badges.png`).
...
Skipping Profile costs a user `@username`, lifetime sets and volume, and the door to Settings. Only the door is load-bearing: 1 of 2 controls, and flow 8, the only flow through, passes straight past everything else in 966 ms. **Refine** holds only if Profile takes the account state it hides (plan, trial, sync, sign-out) and drops stats other pages own; otherwise name the fifth tab Settings and fold identity into its header.
