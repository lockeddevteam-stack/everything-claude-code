# Function Audit — Review (`review`, L15965)

## 1. Purpose
Review is the post-workout receipt: it restates what was just logged, optionally takes a feel rating and an AI insight, and banks the session to history.
...
| Retry after insight failure | — | dead | no control on the ai step |
| ThrowbackCard | 16536 | hidden, redundant | 1-in-5 random (15973), ai step only; Progress has it too (28645) |
| "SAVE WORKOUT" | 16584 | working | `review-populated-ai-insight.png` |
| Two-step Discard, "Keep it" | 16606 | working | `review-populated-discard-armed.png` |
...
Taps 3 (2 + Resume), expected 3, 1076 ms (`flow-metrics.jsonl` flow 3). Reflection path 7 taps, insight path 8. Hesitation: the accent button is "HOW DID IT FEEL?" while the exit is the quiet outline below it; and on the ai step a failed insight has no retry, so the choices are the canned line or Discard.

Entry hazard: stale `lk_activeWorkoutRows` routes `startWorkout` to Review, not the log (57402, `user-flows.md` L13).
...
|---|---|---|---|---|
| Empty | `review-empty.png` | yes | diff vs `review-populated.png` (matched frame): 0 Sets, 0 kg, card gone | **wrong** — "WORKOUT COMPLETE" over an unlogged session; no copy, no guard |
...
| Task success | 3 | Flow 3 completes first attempt (`flow-3-home-saved.png`, 1076 ms) but its spec fails on D1; the insight branch has no retry, recovery means discarding (`review-error.png`) |
...
| Error recovery | 3 | `review-error.png`: cause named ("Connection error…"), workout kept and saveable; no retry, and the failure is dressed as a genuine insight |
...
**Keep**
- One-tap save from the summary, 0 screen changes, and stat tiles correct against seed (`flow-3.json`, `review-populated.png`).
- Two-step Discard with "Keep it" (`review-populated-discard-armed.png`).
...
- Guard `save()` at 16012 on `totalSets === 0`; give empty its own copy and one Discard action (`review-empty.png`).
- Make the completing action primary, demote reflection (`review-populated.png`).
- Replace the canned fallback (16112) with a named failure plus Retry, save still available; give loading real copy (`review-error.png`, `review-loading.png`).
- Reconcile warm-up counting (15985 vs 16254).
...
- The five-rating gate as the primary path (16348): five taps of self-report whose only visible use is a network call that may not answer. Five seconds after racking, a lifter wants the comparison — this session against the last, per lift, and any PR — computable locally (`prs` 10760, `history`). Reflection is an option, not the gate.
