...
| 6 questions, tappable options | CoachInterview L49743, L49827 | working (`coach-setup-populated-interview.png` "1 of 6"; `-interview-answered.png` "2 of 6") |
| Typed free answer + Next | L49845-L49866 | working |
| POST → instructions draft | L49772 | working (measured: preview reached, see walkthrough) |
...
| Instructions textarea, 2000 counter, Save | L49523-L49551 | working (`coach-setup-populated.png` 57/2000) |
...
| Memory list + forget ×, Add, Clear all (two-tap) | L49640, L49666, L49690 | working (`-populated-add-memory.png`) |
| 9 data-visibility toggles | L49703-L49735 | working (18 switch clicks all responded, baseline) |
...
| Interview Q1 goal | L49744 | redundant: Settings body-stats `goal` (L32771) already feeds the coach (L49045) |
| "Rename your coach" pencil (header) | header, 10x12 px | hidden: baseline D12 lists it among 179 sub-44px targets |
...
No flow in `user-flows.md` touches this pane; the nearest is Flow 5 (Coach message, 2 taps, 795 ms, `flow-5.json` 1442 ms Nav "Coach" → 2947 ms reply), which proves the coach answers with **zero** setup — this page gates nothing.
...
|---|---|---|---|
| Empty | `coach-setup-empty.png` | present — same offset as `-populated.png`; placeholder "Goals, injuries, preferences…", `0 / 2000`, and "Nothing remembered yet." (baseline empty check) | correct, specific |
| Loading | `coach-setup-loading.png` | present — "Writing your instructions…" pulsing, replacing the question block | correct, specific (shortfall: no cancel, unlike chat's Stop) |
| Error | `coach-setup-error.png` | present — "The coach is down. Not your fault — your answers are kept." + Retry | correct for the induced abort, names cause and action |
...
|---|---|---|
| Purpose clarity | 4 | `coach-setup-populated.png`: four labelled sections, exactly one accent control ("Let the coach interview you"); Save renders disabled-grey. Shortfall: the pane carries two jobs (persona and privacy) behind a tab labelled "Setup". |
| Feature completeness | 3 | Table above: every visible control responds (29/30 baseline), but "Edit first" loses the draft, Escape does not close (measured), and three interview questions duplicate the style picker, MEMORY and Settings `goal`. |
| Task success | 3 | Interview runs to preview in my measured pass (`-populated-interview.png` → `-interview-answered.png` → `coach-setup-loading.png`); the Edit-first and mistap paths need a restart-or-retype workaround; no user-flow covers the page. |
| Speed | 3 | Measured 9 taps to preview, 10 to saved, 12 to a coached answer, vs 8 for the same six questions with no tab hunt — within 2×. |
| State handling | 5 | A=4, M=0, W=0, G=0 per the round-3 band table: empty, loading, error and populated each distinguishable at matched offset and each specific (captures above). |
| Data correctness | 5 | 3 checks vs `seed-data.json`: `lk_coachInstructions` 57 chars → "57 / 2000" (`-populated.png`); `lk_coachStyle:"direct"` → Direct card outlined (`-scrolled-2.png`); memory text and "added by you, 8/25/2026" verbatim (`-scrolled-3.png`). |
| Error recovery | 4 | `coach-setup-error.png` names the cause, offers Retry, and keeps the answers. Shortfall: dismissing instead of retrying destroys all six answers (measured reopen → "1 of 6"). |
...
- Give the preview a capture and the loading phase a cancel.
