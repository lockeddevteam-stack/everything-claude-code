...
|---|---|---|---|
| Back to Train | L56244 | working | P; crawl responded |
| Favorites/Log/History tabs | L56229 | working | P, L, H |
| Initial tab = favorites else log | L56217 | working | P (Favorites) vs E (Log) |
| Favourite chip + row, one-tap log | L55636/55668 | working | P; 2 measured taps to save |
| Edit mode: reorder, rename, delete-confirm | L55682-55764 | hidden | P shows only "Edit"; four controls behind it, none crawled |
| Favorites empty + "Browse activities" | L55601 | working | EF |
| 41-tile catalogue, 4 groups, search, star | L56499-56590 | working | L ("41 activities") |
...
| History list | L55924 (`type==="cardio"`) | **redundant** | Train Hub History renders the same `lk_history` rows with the same meta line (L6927); its crawl clicked "Treadmill run Sep 6", "Rowing erg Aug 30" |
| `lk_cardioPrefs` (distUnit, weekly target, zones) | L5645-5656 | **dead** | `saveCardioPrefs` has zero callers; unit comes from `useKg` (L55928, L56378); the 150-min target appears nowhere in H |
...
A = 2. Loading and error N/A: no network (page-map 2.12 "L/X n/a"; `screenshots/index.md` L445-446).

| State | Capture | Present | Quality |
|---|---|---|---|
| Empty (history) | EH | yes — "No cardio yet", absent from every populated shot (test 2b) | generic: correct, zero action controls |
...
Against `tests/fixtures/seed-data.json` → `lk_history` (4 cardio records), read off H: Furthest 9 km = `distanceM 9000` ✓; Best pace 5:46/km = 1800 s ÷ 5.2 ✓; split 2:10 = 1200 ÷ 9.2 ✓; zones 45+25+50 = 120 min = 30+20+45+25 ✓; all four rows match name/date/minutes/distance/calories ✓. Shortfall: distance unit derives from the **mass** unit (`p.useKg` L55928) while `lk_cardioPrefs.distUnit` exists and is unwritable.

## 7. Rubric scores

| criterion | score | evidence |
|---|---|---|
| Purpose clarity | 4 | P: "CARDIO" + 3 tabs, one job; shortfall — landing tab flips (L56217) and the favourites view offers three equally-weighted candidates (chip, row, "Log something else") |
| Feature completeness | 3 | Feature table: core works; "Yours" dead (L56500), `saveCardioPrefs` uncalled (L5656), brand step redundant (S), History duplicated |
| Task success | 3 | Primary log passes first attempt (measured, 2 taps); D8 back fails (`global.spec.ts` "reach cardio"); a cardio row opened from Train Hub History renders header + Delete only (measured) |
| Speed | 4 | Measured from the Cardio screen: 2 taps to save a favourite, 3 with the mandatory "Done" = best-known +1; browse path 5, one of them spent on S |
| State handling | 4 | Band table, A=2, M=0 W=0 G=1: EH correct but actionless; EF and P correct |
| Data correctness | 4 | 5 seed checks above against H; shortfall — distance unit bound to `useKg` |
| Error recovery | N/A | No network branch (page-map 2.12); the only failure path is a validation toast, absent from the 19 captures |
